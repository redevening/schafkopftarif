...
function calculatePlayPrice(play) {
    ...
    return signum * Math.pow(2, play.klopf) * (base + play.extraPrice * (+play.schneider + +play.schwarz + play.lauf))
}
...
function calculateEarningsForPlayer(player, plays) {
    ...
    .map(play => {
        const isPlayer = play.p1?.name === player.name || play.p2?.name === player.name
